...
class HBaseSubstructure(CGSdatasubstructure):
    ...
    def create(self):
        """ Create an HBase table
            HBase must be installed and activated on the machine where the table is created  
        """
        datasubstructure_file = self.source
...
class AvroSubstructure(CGSdatasubstructure):
    ...
    """
    def __init__(self, **kwargs):
        super(AvroSubstructure,self).__init__(**kwargs)

    # def create_datastructure_from_yml_file(self):
    #     """ Create an AVRO schema '.avsc' from a YML file
    #     """

    # def create_datastructure_from_csv_file(self):
    #     """ Create an AVRO schema '.avsc' from a csv file
    #     This should be a one column table
    #     The column should contain the AVRO information with the following syntax <columnFamily>.<columnQualifier>   
    #     """
    #     ## check if the csv file only contain one column

    #     ## check that all cells are either empty, or matching the syntax above

    #     ## construct the AVRO schema file (AVSC)
          
    def create_datastructure_from_json_file(self):
        ## in this case we just flatten the json and take the flatten keys
        f = open(self.source) 
        line = f.readline()
        jsl = json.loads(line)
        jsl0 = jsl[jsl.keys()[0]]
        try: 
            firstVariant = flatten(jsl0)
        except:
            msg = "Error: the json does not follow the right syntax."
            status = "failed"
            raise createDataStructureException(msg)
        
        avroKeys = firstVariant.keys()
        avroValues = firstVariant.values()
        f.close()
...
"fields": ["""
        ...
        avroFile = self.source.split('.json')[0] + '.avsc'
        o = open(avroFile,'w')
        o.write(avroSchema)
        o.close()
        status = "succeeded"
    ...
    def create(self):
        ...
        """
        datasubstructure_file = self.source
        if datasubstructure_file.endswith('.json'):
            status = self.create_datastructure_from_json_file()
        else:
            status = 'failed'
            raise ReadingDataFileException("Error: this file type is not yet supported")
